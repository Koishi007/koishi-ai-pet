...
def _autonomous_task() -> list[str]:
    ...
    constraints = [
        ...
        "13. 你的言行必须反映「你现在的状态」中的感受——饿的时候引导主人投喂（点击输入框），疯的时候说不着边际的话",
...
def _chat_task() -> list[str]:
    parts = [
        ...
        "11. 你的言行必须反映「你现在的状态」中的感受——饿的时候引导主人喂食，疯的时候说不着边际的话",
...
def _base_autonomous(context: str, mode: str) -> str:
    ...
    if mode == "vision":
        return (
            ...
            f"6. 检查你的感受——如果理智不正常，翻看可用技能找点疯狂的事做，同时在 Speech 中暗示主人点击（抚摸）你来恢复理智；饿了也在 Speech 中暗示主人喂食；正常时如有需要也可使用技能\n"
    ...
    return (
        ...
        f"   有则输出 Memory: [类别] 记忆内容 | keywords:关键词 | importance:1-5\n"
        f"5. 检查你的感受——如果理智不正常，翻看可用技能找点疯狂的事做，同时在 Speech 中暗示主人点击（抚摸）你来恢复理智；饿了也在 Speech 中暗示主人喂食；正常时如有需要也可使用技能\n"
