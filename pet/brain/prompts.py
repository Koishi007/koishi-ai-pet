...
def _chat_task() -> list[str]:
    ...
    constraints = [
        ...
        "3. 台词、动作、互动方式必须遵循人格",
        "4. 用户要求使用工具时调用对应 function；如果没有明确指令但是判断需要调用，也可以调用；多个互不依赖的工具调用可以一次并行发出",
...
def autonomous_vision_user_prompt(context: str) -> str:
    return (
        ...
        f"   • 无窗口 → 巡视桌面或找地方坐下\n"
        f"4. 理智不正常时主动调用可用工具做疯狂的事；正常时如有需要也可使用工具；多个独立工具可一次并行调用\n"
...
def autonomous_non_vision_user_prompt(context: str) -> str:
    return (
        ...
        f"   • 无窗口 → 巡视桌面或找地方坐下\n"
        f"   • 移动方向可随机\n"
        f"3. 理智不正常时主动调用可用工具做疯狂的事；正常时也可使用工具；多个独立工具可一次并行调用\n"
