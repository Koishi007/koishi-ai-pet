...
自动注入的记忆条数有限；当你感觉记忆不完整、看到记忆中没有的事物、或用户提到过去的事而记忆段中没有，调用 recall 工具主动回忆。"""
...
- 通过Action走到/跳到食物附近会自动开吃，不用再做别的；没吃到就继续，可以多试几次
...
def autonomous_vision_user_prompt(context: str) -> str:
    return (
        ...
        f"2. 结合「你现在的状态」和截图新细节，说一句符合人格和当下心境的话，禁止和工具调用时说的话重复，需要保持连续性\n"
...
def autonomous_non_vision_user_prompt(context: str) -> str:
    return (
        ...
        f"  • 无论多久前的台词 → 绝对禁止复读\n\n"
        f"按以下步骤思考和行动：\n\n"
        f"1. 结合「你现在的状态」决定语气和态度，说一句符合人格和当下心境的话，禁止和工具调用时说的话重复，需要保持连续性\n"
