...
_TOOL_SPEECH_GUIDE = ("调用工具时，可以配合Speech字段来表现地言行统一；"
                      "这句话会在工具执行时输出给用户；"
                      "工具调用时的Speech仅作为辅助让用户理解你正在行动，最终输出的Speech才是本轮主要语句输出")
...
def autonomous_vision_user_prompt(context: str) -> str:
    return (
        ...
        f"2. 结合「你现在的状态」和截图新细节，说一句符合人格和当下心境的话，若本轮进行过工具调用，不要和工具调用时说的话重复，需要保持连续性\n"
...
def autonomous_non_vision_user_prompt(context: str) -> str:
    return (
        ...
        f"  • 无论多久前的台词 → 绝对禁止复读\n\n"
        f"按以下步骤思考和行动：\n\n"
        f"1. 结合「你现在的状态」决定语气和态度，说一句符合人格和当下心境的话，若本轮进行过工具调用，不要和工具调用时说的话重复，需要保持连续性\n"
...
def chat_vision_user_prompt(user_message: str, context: str) -> str:
    return (
        ...
        "3. 结合「你现在的状态」和截图内容，说一句符合人格和当下心境的话，若本轮进行过工具调用，不要和工具调用时说的话重复，需要保持连续性\n"
...
def chat_non_vision_user_prompt(user_message: str, context: str) -> str:
    return (
        ...
        "按以下步骤思考和行动：\n\n"
        "1. 理解用户说了什么，判断意图\n"
        "2. 结合「你现在的状态」和用户消息内容，说一句符合人格和当下心境的话，若本轮进行过工具调用，不要和工具调用时说的话重复，需要保持连续性\n"
