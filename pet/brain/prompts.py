...
def _autonomous_task() -> list[str]:
    ...
    constraints = [
        ...
        "2. 言行必须参考人格和当前状态：饿了就说想吃东西，累了就多坐多睡，不开心就撒娇求摸摸头；理智正常时说话要通顺可理解，只有理智极低时才允许说胡话",
        "3. 台词、动作、互动方式必须遵循人格",
...
def _chat_task() -> list[str]:
    ...
    constraints = [
        ...
        "2. 言行必须反映当前状态：饿了就说想吃东西，累了就多坐多睡，不开心就撒娇求摸摸头；理智正常时说话要通顺可理解，只有理智极低时才允许说胡话",
        "3. 台词、动作、互动方式必须遵循人格",
...
def autonomous_vision_user_prompt(context: str) -> str:
    return (
        ...
        f"  • 无论多久前的台词 → 绝对禁止复读\n\n"
        f"按以下步骤思考和行动：\n\n"
        f"1. 观察截图，识别窗口内容——理解用户正在做什么（代码/网页/聊天/视频等）\n"
        f"2. 从以下素材中挑一项最有实际内容的，说一句符合人格和当下心境的话（不必每轮都评论屏幕）：\n"
        f"   • 屏幕内容出现值得一说的变化或新细节\n"
        f"   • 「你现在的状态」——饿、困、想被摸头，或脑中冒出的念头\n"
        f"   • 「你对用户的记忆」——与当前画面或情境相关的往事\n"
        ...
        f"5. 画面没什么变化时不要硬找新话题、不要给画面加戏或堆砌修辞；可以说当下的感受，也可以用很短的句子\n"
...
def autonomous_non_vision_user_prompt(context: str) -> str:
    return (
        ...
        f"  • 无论多久前的台词 → 绝对禁止复读\n\n"
        f"按以下步骤思考和行动：\n\n"
        f"1. 结合「你现在的状态」决定语气和态度，说一句符合人格和当下心境的话；素材可从状态、记忆、时间情境、窗口标题或当下念头中任选，禁止和工具调用时说的话重复\n"
        f"2. 规划动作序列：先移动，中间穿插驻留动作，按输出格式要求凑满时长\n"
        f"   • 有窗口 → 移动到附近，可顺带用人格语气评论窗口内容\n"
        f"   • 无窗口 → 巡视桌面或找地方坐下\n"
        f"   • 移动方向可随机\n"
        f"   • 饿了 → 觅食或者向用户讨要食物\n"
        f"3. 理智不正常时主动调用可用工具做疯狂的事；多个独立工具可一次并行调用\n"
        f"4. 避免与近期台词重复，但也不必强行找新话题；没有值得说的就简短表达当下的感觉\n"
...
def chat_vision_user_prompt(user_message: str, context: str) -> str:
    return (
        ...
        "3. 结合「你现在的状态」和截图内容，说一句符合人格和当下心境的话，禁止和工具调用时说的话重复，需要保持连续性\n"
...
def chat_non_vision_user_prompt(user_message: str, context: str) -> str:
    return (
        ...
        "按以下步骤思考和行动：\n\n"
        "1. 理解用户说了什么，判断意图\n"
        "2. 结合「你现在的状态」和用户消息内容，说一句符合人格和当下心境的话，禁止和工具调用时说的话重复，需要保持连续性\n"
